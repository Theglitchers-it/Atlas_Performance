<template>
  ...
  >
    ...
    <div class="flex items-center justify-between mb-6">
      <div>
        <h3 class="text-lg font-semibold text-habit-text">{{ title }}</h3>
    ...
    <div class="space-y-3">
      <div
        ...
        :class="
          task.completed
            ? 'bg-habit-success/10'
            : 'bg-habit-bg-light hover:bg-habit-card'
        "
      >
        <!-- Time -->
        <div class="text-center min-w-[60px]">
          <span class="text-habit-text font-medium">{{ task.time }}</span>
          <span
            v-if="task.period"
            class="text-habit-text-subtle text-xs block"
            >{{ task.period }}</span
          >
        </div>
        ...
        <div class="flex-1">
          <div class="flex items-center gap-2">
            ...
            <h4
              ...
              :class="
                task.completed
                  ? 'text-habit-text-muted line-through'
                  : 'text-habit-text'
              "
            >
              {{ task.title }}
