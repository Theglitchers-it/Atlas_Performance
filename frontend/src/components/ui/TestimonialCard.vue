<template>
  ...
  >
    <!-- Quote -->
    <p class="text-habit-text text-lg leading-relaxed pl-8 mb-6">
      {{ quote }}
    </p>
    ...
    <div class="flex items-center gap-4">
      <!-- Avatar -->
      <div class="relative">
        <img v-if="avatar" :src="avatar" :alt="name" class="avatar avatar-lg" />
        <div
          v-else
        ...
        <div
          v-if="online"
          class="absolute bottom-0 right-0 w-3 h-3 bg-habit-success rounded-full border-2 border-habit-card"
        ></div>
      </div>

      <!-- Info -->
      <div class="flex-1">
        <h4 class="text-habit-text font-semibold">{{ name }}</h4>
    ...
    <div
      v-if="videoThumbnail"
      class="mt-6 relative rounded-xl overflow-hidden cursor-pointer group"
    ...
    >
      ...
      >
        <div
          class="w-14 h-14 rounded-full bg-habit-orange flex items-center justify-center"
        >
          <svg
    ...
    <div
      ...
      class="mt-6 pt-6 border-t border-habit-border grid grid-cols-3 gap-4"
    >
      <div v-for="(stat, index) in stats" :key="index" class="text-center">
        <div class="text-xl font-bold text-habit-text">{{ stat.value }}</div>
