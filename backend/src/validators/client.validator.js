...
const createClientSchema = Joi.object({
    ...
    secondaryGoals: Joi.array().items(Joi.string()).allow(null),
    medicalNotes: Joi.string().allow('', null),
    dietaryRestrictions: Joi.array().items(Joi.string()).allow(null),
    ...
    sportHistory: Joi.string().max(2000).allow('', null),
    occupationType: Joi.string().valid('sedentary','moderately_active','active','highly_active').allow(null),
    dailyStepsAvg: Joi.number().integer().min(0).max(100000).allow(null),
    jointPainAreas: Joi.array().items(Joi.string().max(50)).allow(null),
    previousDiets: Joi.string().max(2000).allow('', null),
    foodAllergies: Joi.string().max(1000).allow('', null),
    currentDietPhase: Joi.string().valid('cut','normocaloric','bulk','free').allow(null),
    baselineStressLevel: Joi.number().integer().min(1).max(10).allow(null),
    mealsPerDayHabit: Joi.number().integer().min(1).max(10).allow(null),
